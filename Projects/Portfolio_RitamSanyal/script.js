document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (e.target === modal || e.target === closeModal || e.target.closest('.close-modal')) {
        ...
        contactForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok && result.success) {
                    ...
                    throw new Error(result.message || 'Form submission response was not ok.');
                }
            } catch (err) {
                console.error(err);
                showStatus(err.message || 'Oops! Something went wrong. Please try again later.', 'error');
            } finally {
                // Re-enable button
                if (submitBtn) {
